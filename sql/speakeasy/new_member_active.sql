...
INSERT INTO speakeasy_petition_metrics (activity, campaign_id, npeople)
  ...
  WHERE a.status_id=9 
    ...
    AND a_act.activity_date_time >= DATE_ADD(NOW(), INTERVAL -1 MONTH)
  GROUP BY a.campaign_id;

-- Number of new people who were active 2 months later
INSERT INTO speakeasy_petition_metrics (activity, campaign_id, npeople)
  SELECT 
    'new_active_2mlater', a.campaign_id, COUNT(DISTINCT ac.contact_id) 
  FROM civicrm_activity a 
  JOIN civicrm_activity_contact ac ON ac.activity_id=a.id 
  JOIN civicrm_activity_contact ac_act ON ac.contact_id=ac_act.contact_id
  JOIN civicrm_activity a_act ON a_act.id=ac_act.activity_id
  WHERE a.status_id=9 
    ...
    AND (a_act.activity_date_time 
      BETWEEN DATE_ADD(a.activity_date_time, INTERVAL 1 MONTH)
          AND DATE_ADD(a.activity_date_time, INTERVAL 2 MONTH))
  GROUP BY a.campaign_id;