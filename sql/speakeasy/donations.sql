...
INSERT INTO speakeasy_petition_metrics (activity, campaign_id, status, npeople)
  ...
  WHERE contribution_status_id IN (1, 5) AND is_test = 0 AND campaign_id IS NOT NULL
...
-- Donations count on pending status (like SEPA)
INSERT INTO speakeasy_petition_metrics (activity, campaign_id, status, npeople)
  SELECT
    'donation_pending', campaign_id, currency, count(id) donations_count
  FROM civicrm_contribution
  WHERE contribution_status_id = 2 AND is_test = 0 AND campaign_id IS NOT NULL
  GROUP BY campaign_id, currency;

-- Donations amount on pending status (like SEPA)
INSERT INTO speakeasy_petition_metrics (activity, campaign_id, status, npeople)
  SELECT
    'donation_pending_amount', campaign_id, currency, sum(total_amount)
  FROM civicrm_contribution
  WHERE contribution_status_id = 2 AND is_test = 0 AND campaign_id IS NOT NULL
  GROUP BY campaign_id, currency;
