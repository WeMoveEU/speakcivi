<?php
/*
 * Settings metadata file
 */

return array(
  'opt_in' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'opt_in',
    'type' => 'Boolean',
    'quick_form_type' => 'YesNo',
    'default' => 1,
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Opt-in',
    'description' => "Opt-in",
    'help_text' => "Opt-in",
  ),
  'url_speakout' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'url_speakout',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'act.wemove.eu/campaigns',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Url to Speakout API',
    'description' => "Url to Speakout API",
    'help_text' => "Url to Speakout API",
  ),
  'gdpr_privacy_pack_version' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'gdpr_privacy_pack_version',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => '2.0.0',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'GDPR Privacy Pack Version',
    'description' => "GDPR Privacy Pack Version",
    'help_text' => "GDPR Privacy Pack Version",
  ),
  'anonymous_id' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'anonymous_id',
    'type' => 'Integer',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 0,
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Contact Id for anonymous contact (without email)',
    'description' => 'Contact Id for anonymous contact (without email)',
    'help_text' => 'Contact Id for anonymous contact (without email)',
  ),
  'group_id' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'group_id',
    'type' => 'Integer',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 42,
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Default group Id',
    'description' => 'Default group Id',
    'help_text' => 'Default group Id',
  ),
  'no_member_group_id' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'no_member_group_id',
    'type' => 'Integer',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 1148,
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Group Id for contacts belonging to our partners (NoMember)',
    'description' => 'Group Id for contacts belonging to our partners (NoMember)',
    'help_text' => 'Group Id for contacts belonging to our partners (NoMember)',
  ),
  'no_member_campaign_type' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'no_member_campaign_type',
    'type' => 'Integer',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 10,
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Campaign type for No Member mode',
    'description' => 'Campaign type for No Member mode',
    'help_text' => 'Campaign type for No Member mode',
  ),
  'default_language_group_id' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'default_language_group_id',
    'type' => 'Integer',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 47,
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Default language group Id',
    'description' => 'Default language group Id',
    'help_text' => 'Default language group Id',
  ),
  'language_group_name_suffix' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'language_group_name_suffix',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => '-language-activists',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Suffix of language group name',
    'description' => "Suffix of language group name",
    'help_text' => "Suffix of language group name",
  ),
  'language_tag_name_prefix' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'language_tag_name_prefix',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'can speak ',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Prefix of language tag name',
    'description' => "Prefix of language tag name",
    'help_text' => "Prefix of language tag name",
  ),
  'default_language' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'default_language',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'en_GB',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Default language',
    'description' => "Default language",
    'help_text' => "Default language",
  ),
  'field_template_id' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'field_template_id',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'custom_3',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Custom field for campaign\'s message template',
    'description' => "Custom field for campaign's message template",
    'help_text' => "Custom field for campaign's message template",
  ),
  'field_language' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'field_language',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'custom_4',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Custom field for campaign\'s language',
    'description' => "Custom field for campaign's language",
    'help_text' => "Custom field for campaign's language",
  ),
  'field_sender_mail' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'field_sender_mail',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'custom_5',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Custom field for campaign\'s sender mail',
    'description' => "Custom field for campaign's sender mail",
    'help_text' => "Custom field for campaign's sender mail",
  ),
  'field_url_campaign' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'field_url_campaign',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'custom_8',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Custom field for URL of campaign on speakout',
    'description' => "Custom field for URL of campaign on speakout",
    'help_text' => "Custom field for URL of campaign on speakout",
  ),
  'field_language_form_url' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'field_language_form_url',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => '',
    'add' => '5.13',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Custom field for URL of the language conversion form',
    'description' => "Custom field for URL of the language conversion form",
    'help_text' => "Custom field for URL of the language conversion form",
  ),
  'field_utm_campaign' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'field_utm_campaign',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'custom_11',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Custom field for UTM Campaign',
    'description' => "Custom field for UTM Campaign",
    'help_text' => "Custom field for UTM Campaign",
  ),
  'field_twitter_share_text' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'field_twitter_share_text',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'custom_16',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Custom field for text used in twitter sharing',
    'description' => "Custom field for text used in twitter sharing",
    'help_text' => "Custom field for text used in twitter sharing",
  ),
  'field_subject_new' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'field_subject_new',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'custom_17',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Custom field for subject for the emails to the contacts that aren\'t already members and need to confirm their signature',
    'description' => 'Custom field for subject for the emails to the contacts that aren\'t already members and need to confirm their signature',
    'help_text' => 'Custom field for subject for the emails to the contacts that aren\'t already members and need to confirm their signature',
  ),
  'field_subject_current' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'field_subject_current',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'custom_18',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Custom field for subject for the emails to the contacts that are already members and don\'t need to confirm their signature',
    'description' => 'Custom field for subject for the emails to the contacts that are already members and don\'t need to confirm their signature',
    'help_text' => 'Custom field for subject for the emails to the contacts that are already members and don\'t need to confirm their signature',
  ),
  'field_message_new' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'field_message_new',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'custom_19',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Custom field for body for the emails to the contacts that aren\'t already members and need to confirm their signature',
    'description' => 'Custom field for body for the emails to the contacts that aren\'t already members and need to confirm their signature',
    'help_text' => 'Custom field for body for the emails to the contacts that aren\'t already members and need to confirm their signature',
  ),
  'field_message_current' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'field_message_current',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'custom_20',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Custom field for body for the emails to the contacts that are already members and don\'t need to confirm their signature',
    'description' => 'Custom field for body for the emails to the contacts that are already members and don\'t need to confirm their signature',
    'help_text' => 'Custom field for body for the emails to the contacts that are already members and don\'t need to confirm their signature',
  ),
  'field_redirect_confirm' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'field_redirect_confirm',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'custom_44',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Redirect after confirm action',
    'description' => 'Redirect after confirm action',
    'help_text' => 'Redirect after confirm action',
  ),
  'field_redirect_optout' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'field_redirect_optout',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'custom_45',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Redirect after optout action',
    'description' => 'Redirect after optout action',
    'help_text' => 'Redirect after optout action',
  ),
  'field_activity_source' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'field_activity_source',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'custom_27',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Custom field for source activity',
    'description' => "Custom field for source activity",
    'help_text' => "Custom field for source activity",
  ),
  'field_activity_medium' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'field_activity_medium',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'custom_28',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Custom field for medium activity',
    'description' => "Custom field for medium activity",
    'help_text' => "Custom field for medium activity",
  ),
  'field_activity_campaign' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'field_activity_campaign',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'custom_26',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Custom field for campaign activity',
    'description' => "Custom field for campaign activity",
    'help_text' => "Custom field for campaign activity",
  ),
  'field_contact_source' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'field_contact_source',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'custom_22',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Custom field for source contact',
    'description' => "Custom field for source contact",
    'help_text' => "Custom field for source contact",
  ),
  'field_contact_medium' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'field_contact_medium',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'custom_23',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Custom field for medium contact',
    'description' => "Custom field for medium contact",
    'help_text' => "Custom field for medium contact",
  ),
  'field_contact_campaign' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'field_contact_campaign',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'custom_21',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Custom field for campaign contact',
    'description' => "Custom field for campaign contact",
    'help_text' => "Custom field for campaign contact",
  ),
  'field_tracking_codes_source' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'field_tracking_codes_source',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'custom_37',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Custom field for tracking_codes.utm_source',
    'description' => "Custom field for tracking_codes.utm_source",
    'help_text' => "Custom field for tracking_codes.utm_source",
  ),
  'field_tracking_codes_medium' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'field_tracking_codes_medium',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'custom_35',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Custom field for tracking_codes.utm_medium',
    'description' => "Custom field for tracking_codes.utm_medium",
    'help_text' => "Custom field for tracking_codes.utm_medium",
  ),
  'field_tracking_codes_campaign' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'field_tracking_codes_campaign',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'custom_36',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Custom field for tracking_codes.utm_campaign',
    'description' => "Custom field for tracking_codes.utm_campaign",
    'help_text' => "Custom field for tracking_codes.utm_campaign",
  ),
  'field_tracking_codes_content' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'field_tracking_codes_content',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'custom_36',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Custom field for tracking_codes.utm_content',
    'description' => "Custom field for tracking_codes.utm_content",
    'help_text' => "Custom field for tracking_codes.utm_content",
  ),
  'field_contribution_source' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'field_contribution_source',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'custom_30',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Custom field for contribution.utm_source',
    'description' => "Custom field for contribution.utm_source",
    'help_text' => "Custom field for contribution.utm_source",
  ),
  'field_contribution_medium' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'field_contribution_medium',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'custom_31',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Custom field for contribution.utm_medium',
    'description' => "Custom field for contribution.utm_medium",
    'help_text' => "Custom field for contribution.utm_medium",
  ),
  'field_contribution_campaign' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'field_contribution_campaign',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'custom_33',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Custom field for contribution.utm_campaign',
    'description' => "Custom field for contribution.utm_campaign",
    'help_text' => "Custom field for contribution.utm_campaign",
  ),
  'field_contribution_content' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'field_contribution_content',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'custom_32',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Custom field for contribution.utm_content',
    'description' => "Custom field for contribution.utm_content",
    'help_text' => "Custom field for contribution.utm_content",
  ),
  'field_consent_date' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'field_consent_date',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'custom_56',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Custom field for GDPR Consent date',
    'description' => "Custom field for GDPR Consent date",
    'help_text' => "Custom field for GDPR Consent date",
  ),
  'field_consent_version' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'field_consent_version',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'custom_57',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Custom field for GDPR Consent version',
    'description' => "Custom field for GDPR Consent version",
    'help_text' => "Custom field for GDPR Consent version",
  ),
  'field_consent_language' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'field_consent_language',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'custom_62',
    'add' => '4.7',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Custom field for GDPR Consent language',
    'description' => "Custom field for GDPR Consent language",
    'help_text' => "Custom field for GDPR Consent language",
  ),
  'field_consent_campaign_id' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'field_consent_campaign_id',
    'type' => 'Integer',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'custom_58',
    'add' => '4.7',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Custom field for GDPR Consent Campaign Id',
    'description' => "Custom field for GDPR Consent Campaign Id",
    'help_text' => "Custom field for GDPR Consent Campaign Id",
  ),
  'field_consent_utm_source' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'field_consent_utm_source',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'custom_59',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Custom field for GDPR Consent utm_source',
    'description' => "Custom field for GDPR Consent utm_source",
    'help_text' => "Custom field for GDPR Consent utm_source",
  ),
  'field_consent_utm_medium' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'field_consent_utm_medium',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'custom_60',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Custom field for GDPR Consent utm_medium',
    'description' => "Custom field for GDPR Consent utm_medium",
    'help_text' => "Custom field for GDPR Consent utm_medium",
  ),
  'field_consent_utm_campaign' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'field_consent_utm_campaign',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'custom_61',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Custom field for GDPR Consent utm_campaign',
    'description' => "Custom field for GDPR Consent utm_campaign",
    'help_text' => "Custom field for GDPR Consent utm_campaign",
  ),
  'field_campaign_consent_ids' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'field_campaign_consent_ids',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 'custom_79',
    'add' => '4.7',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Custom field for Consent IDs at campaign',
    'description' => "Custom field for Consent IDs at campaign",
    'help_text' => "Custom field for Consent IDs at campaign",
  ),
  'from' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'from',
    'type' => 'String',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => '"Xavier - WeMove Europe" <info@wemove.eu>',
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'From',
    'description' => "From",
    'help_text' => "From",
  ),
  'activity_type_join' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'activity_type_join',
    'type' => 'Integer',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 57,
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Activity type ID used for marking contacts who are added to group.',
    'description' => 'Activity type ID used for marking contacts who are added to group.',
    'help_text' => 'Activity type ID used for marking contacts who are added to group.',
  ),
  'activity_type_leave' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'activity_type_leave',
    'type' => 'Integer',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 56,
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Activity type ID used for marking contacts who are removed from group.',
    'description' => 'Activity type ID used for marking contacts who are removed from group.',
    'help_text' => 'Activity type ID used for marking contacts who are removed from group.',
  ),
  'threshold_very_old_activity' => array(
    'group_name' => 'Speakcivi API Preferences',
    'group' => 'speakcivi',
    'name' => 'threshold_very_old_activity',
    'type' => 'Integer',
    'quick_form_type' => 'Element',
    'html_type' => 'text',
    'default' => 3,
    'add' => '4.6',
    'is_domain' => 1,
    'is_contact' => 0,
    'title' => 'Threshold in days for very old activity',
    'description' => 'Threshold in days for very old activity',
    'help_text' => 'Threshold in days for very old activity',
  ),
);
