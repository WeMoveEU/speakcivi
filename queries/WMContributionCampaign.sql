...
status.label as status, instrument.label as instrument, 
...
known_signs.value AS signatures 
 from 
(select 
min(receive_date) min_date,max(receive_date) max_date, avg(receive_date) avg_date,
min(c.id) as id, currency,count(*) as nb, sum(total_amount) amount, sum(fee_amount) fee,sum(net_amount) net, payment_instrument_id, contribution_status_id status_id, campaign_id, utm_source_30 as utm_source,utm_medium_31 as utm_medium,utm_campaign_33 as utm_campaign 
