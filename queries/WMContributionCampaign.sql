...
camp.id as campaign_id, camp.parent_id as parent_id, camp.title as camp, 
c.*,
r.value AS recipients, 
known_signs.value AS signatures 
 from 
(select min(id) as c_id, count(*) as nb, sum(total_amount) amount, sum(fee_amount) fee,sum(net_amount) net, payment_instrument_id, contribution_status_id status_id, campaign_id, utm_source_30 as utm_source,utm_medium_31 as utm_medium,utm_campaign_33 as utm_campaign 
