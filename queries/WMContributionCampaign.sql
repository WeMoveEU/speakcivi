select m.name mailing, m.id mailing_id, status.name as status, instrument.name as instrument, camp.id as campaign_id, camp.parent_id as parent_id, camp.title as camp, c.* from 
