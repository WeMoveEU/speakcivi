...
-- -- 28 survey
-- -- 32 petition
SELECT t.contact_id, c2.display_name, c2.created_date, t.activity_count
FROM
  (SELECT
    gc.contact_id, count(ac1.id) AS activity_count
  FROM civicrm_contact c
    JOIN civicrm_group_contact gc ON c.id = gc.contact_id
    JOIN civicrm_activity_contact ac1 ON gc.contact_id = ac1.contact_id and ac1.record_type_id = 2
  ...
  ORDER BY c.created_date) t
  JOIN civicrm_contact c2 ON t.contact_id = c2.id;
